...
export const addCart = (products, Swal) => {
    return async (dispatch) => {
      ...
      const token = localStorage.getItem("token")
      try {
        const result = await axios.patch(`${URL_API}/cart/`, {productId :products.id,total_price}, 
        {
          headers : {
            Authorization : `Bearer ${token}`
          }
  ...
  export const deleteItem = (products) => {
    return async(dispatch) => {
        ...
        const total_price = products.price
        try {
            const response = await axios.patch(`${URL_API}/cart/item`, {productId:products.id, total_price}, 
            {
                headers : {
                    Authorization : `Bearer ${token}`
                }
